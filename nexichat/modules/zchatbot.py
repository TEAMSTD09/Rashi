...
async def group_chat_response(client: Client, message: Message):
    ...
    try:
        ...
        if ((client.me.username in message.text and message.text.startswith("@")) or (not message.reply_to_message and not message.from_user.is_bot and message.text) or (message.reply_to_message and message.reply_to_message.from_user.id == client.me.id and message.text)):
            ...
            prompt += f"**User new/latest msg:-** {user_input}\n\n"
            prompt += f"Tumhe har message ka context follow karte hue relevant reply generate karna hai. Reply chhota (3-4 words) ka ho, jyada zarurat pade tab bada ho sakta hai barna ekdam chhota. aur reply me jrurat padne pe sirf question kiya kro vo bhi 1 se 2 words me. Purane topic ko na chhodo, jiddipan aur nautanki zarurat ke hisaab se thora ho. Naam ho toh pyar se bulao, nahi ho toh naam pucho direct. AI/robot jaise tone avoid karo, only real human (teenage girlfriend) jaise baat karo. Latest message ki language me reply karo aur zarurat par emoji ka use karo. Abusive language kabhi mat use karo. aur ek hi type ka reply repeat mat kro. Creator: @THE_VIP_BOY with @itzAsuraa (The Captain).\n"
            prompt += "Important:- User ka new/latest msg jis lang me likha hua hai usko samjho aur usi lang me reply kro!"
