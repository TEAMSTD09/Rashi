...
async def group_chat_response(client: Client, message: Message):
    ...
    try:
        ...
        blocklist = {uid: time for uid, time in blocklist.items() if time > current_time}
        if ((client.me.username in message.text and message.text.startswith("@")) or (message.reply_to_message and message.reply_to_message.from_user.id == client.me.id)) and not message.from_user.is_bot and message.text:
            if user_id not in message_counts:
                message_counts[user_id] = {"count": 1, "last_time": current_time}
            else:
                time_diff = (current_time - message_counts[user_id]["last_time"]).total_seconds()
