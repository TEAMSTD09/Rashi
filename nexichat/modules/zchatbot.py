...
async def clear_user_history(user_id):
    """Clears the chat history of a specific user."""
    conversation_cache.pop(user_id, None)
    user_data_cache.pop(user_id, None)
    conversation_cache[user_id] = []  # Create a fresh history
...
async def group_chat_response(client: Client, message: Message):
    ...
    try:
        ...
        if not user_id:
            return  # Skip messages without a user ID (e.g., from channels)
        ...
        # Check if user mentioned the bot or replied to its message
        if ((client.me.username in message.text and message.text.startswith("@")) or 
            (message.reply_to_message and message.reply_to_message.from_user.id == client.me.id and message.text)):
            if user_input:
                # Initialize user's conversation history if not present
                ...
                for user_msg, ai_reply in conversation_history[-50:]:
                    prompt += f"User msg:- {user_msg}\nAI reply:- {ai_reply}\n\n"
                prompt += f"User new msg:- {user_input}\nAI new reply give now:-"
                ...
                try:
                    ...
                    if "413" in result or "The request could not be satisfied" in result:
